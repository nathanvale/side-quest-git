...
describe('copyWorktreeFiles', () => {
	...
	test('copies root-level files matching exact pattern', () => {
		...
		expect(fs.readFileSync(path.join(destDir, '.env'), 'utf-8')).toBe('SECRET=abc')
	})
	...
	test('copies files matching recursive pattern', () => {
		...
		expect(fs.existsSync(path.join(destDir, 'plugins', 'git', 'CLAUDE.md'))).toBe(true)
	})
	...
	test('preserves directory structure for recursive copies', () => {
		...
		expect(fs.readFileSync(path.join(destDir, 'a', 'b', 'c', 'CLAUDE.md'), 'utf-8')).toBe('# Deep')
	})
	...
	test('skips excluded directories during recursive copy', () => {
		...
		const copied = copyWorktreeFiles(sourceDir, destDir, ['**/CLAUDE.md'], ['node_modules'])
		expect(copied).toBe(1)
		expect(fs.existsSync(path.join(destDir, 'node_modules', 'pkg', 'CLAUDE.md'))).toBe(false)
	})
	...
	test('copies a directory pattern (e.g., .claude)', () => {
		...
		expect(fs.existsSync(path.join(destDir, '.claude', 'settings.json'))).toBe(true)
		expect(fs.existsSync(path.join(destDir, '.claude', 'CLAUDE.md'))).toBe(true)
	})

	test('returns 0 when no patterns match', () => {
		const copied = copyWorktreeFiles(sourceDir, destDir, ['.env', '.nvmrc'], [])
	...
	test('handles mix of root and recursive patterns', () => {
		...
		const copied = copyWorktreeFiles(sourceDir, destDir, ['.env', '.nvmrc', '**/CLAUDE.md'], [])
	...
	test('does not double-copy files matching multiple recursive patterns', () => {
		...
		const copied = copyWorktreeFiles(sourceDir, destDir, ['**/*.kit', '**/test.kit'], [])
